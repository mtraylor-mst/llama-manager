...
class TestAPIRoutes:
    ...
    def test_list_models_default(self, mock_models, client):
        mock_models.return_value = [{'path': '/models/test.gguf', 'name': 'test.gguf', 'rel': 'test.gguf'}]
...
class TestServerRoutes:
    ...
    def test_status_stopped(self, mock_status, mock_vid, client):
        mock_status.return_value = {'running': False, 'state': 'stopped', 'name': None, 'line': ''}
    ...
    def test_benchmark(self, mock_bench, client):
        mock_bench.return_value = {
            'success': True, 'tps': 50.0, 'tokens_generated': 128,
            'duration_sec': 2.56, 'vram_used_mb': 4096, 'peak_cpu_pct': 85.0, 'error': None
        }
