...
COLUMN_MAP = {
    ...
    'chat_templates': {
        ...
        'prefill_assistant': '--prefill-assistant',
    },
    'checkpoints': {
...
def build_command(version_id):
    ...
    for category in ['model_loading', 'context_batching', 'cpu_threading', 'gpu_device',
                     'memory', 'sampling', 'server', 'speculative', 'chat_templates',
                     'checkpoints', 'logging', 'advanced']:
    ...
    for cv in cvs:
        ...
        if cv.get('layer_range_start') is not None and cv.get('layer_range_end') is not None:
            parts.extend([
