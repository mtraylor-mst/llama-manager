...
def get_server_pid():
    ...
    try:
        ...
        out = subprocess.check_output(
            ["pgrep", "-x", "llama-server"], text=True, stderr=subprocess.DEVNULL
        )
...
FLAG_TO_COLUMN = {
    ...
    "webui": ("server", "webui"),
    "ui": ("server", "webui"),
    "ui-config": ("server", "webui_config"),
    "webui-config": ("server", "webui_config"),
    "ui-config-file": ("server", "webui_config_file"),
    "webui-config-file": ("server", "webui_config_file"),
    "ui-mcp-proxy": ("server", "webui_mcp_proxy"),
    "webui-mcp-proxy": ("server", "webui_mcp_proxy"),
    "tools": ("server", "tools"),
    ...
    "spec-draft-n-max": ("speculative", "spec_draft_n_max"),
    "spec-draft-n-min": ("speculative", "spec_draft_n_min"),
    "spec-draft-p-split": ("speculative", "spec_draft_p_split"),
    "draft-p-split": ("speculative", "spec_draft_p_split"),
    "spec-draft-p-min": ("speculative", "draft_p_min"),
    "spec-draft-threads": ("cpu_threading", "threads_draft"),
    "spec-draft-threads-batch": ("cpu_threading", "threads_batch_draft"),
    "spec-ngram-simple-size-n": ("speculative", "spec_ngram_simple_size_n"),
    "spec-ngram-simple-size-m": ("speculative", "spec_ngram_simple_size_m"),
    "spec-ngram-simple-min-hits": ("speculative", "spec_ngram_simple_min_hits"),
    "spec-ngram-map-k-size-n": ("speculative", "spec_ngram_map_k_size_n"),
    "spec-ngram-map-k-size-m": ("speculative", "spec_ngram_map_k_size_m"),
    "spec-ngram-map-k-min-hits": ("speculative", "spec_ngram_map_k_min_hits"),
    "spec-ngram-map-k4v-size-n": ("speculative", "spec_ngram_map_k4v_size_n"),
    "spec-ngram-map-k4v-size-m": ("speculative", "spec_ngram_map_k4v_size_m"),
    "spec-ngram-map-k4v-min-hits": ("speculative", "spec_ngram_map_k4v_min_hits"),
    "spec-ngram-mod-n-min": ("speculative", "spec_ngram_mod_n_min"),
    "spec-ngram-mod-n-max": ("speculative", "spec_ngram_mod_n_max"),
    "spec-ngram-mod-n-match": ("speculative", "spec_ngram_mod_n_match"),
    ...
    "reasoning-preserve": ("chat_templates", "reasoning_preserve"),
    ...
    "checkpoint-min-step": ("checkpoints", "checkpoint_min_step"),
...
def import_running_config(config_name=None):
    ...
    cfg_name = (
        model_name
        or (config_name if config_name and config_name != "Imported Config" else None)
        or "Imported Config"
    )
