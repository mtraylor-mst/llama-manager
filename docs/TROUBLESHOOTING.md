...
`llama-manager` relies on `screen` and the `llama-server` binary to manage processes.

*   **"Command not found: screen"**: The `screen` utility is not installed. Install it using your package manager (e.g., `sudo apt install screen`).
...
*   **Permission Denied**: Ensure the user running `llama-manager` has permission to execute `screen` and access the `llama-server` binary.
...
3.  **System Logs**: If `screen` or the database is failing, check your system logs (e.g., `journalctl` on Linux).
