...
CATEGORY_FIELDS = {
    'model_loading': [
        ...
        ('mmproj_auto', 'Auto MMProj', 'bool', None),
        ('mmproj_offload', 'Offload MMProj', 'bool', None),
    ],
    'context_batching': [
        ...
        ('cont_batching', 'Continuous Batching', 'bool', None),
        ('cache_prompt', 'Cache Prompts', 'bool', None),
    ...
    'cpu_threading': [
        ...
        ('prio', 'Priority', 'select', [('0', 'normal'), ('-1', 'low'), ('1', 'medium'), ('2', 'high'), ('3', 'realtime')]),
        ('poll', 'Poll Level', 'int', '0-100, default: 50'),
        ('numa', 'NUMA Mode', 'select', [('none', 'none'), ('distribute', 'distribute'), ('isolate', 'isolate'), ('numactl', 'numactl')]),
    ...
    'gpu_device': [
        ...
        ('split_mode', 'Split Mode', 'select', [('none', 'none'), ('layer', 'layer'), ('row', 'row'), ('tensor', 'tensor')]),
        ('tensor_split', 'Tensor Split', 'text', 'e.g. 3,1'),
        ('main_gpu', 'Main GPU', 'int', 'default: 0'),
        ('flash_attn', 'Flash Attention', 'select', [('auto', 'auto'), ('on', 'on'), ('off', 'off')]),
        ('kv_offload', 'KV Offload', 'bool', None),
        ('repack', 'Weight Repack', 'bool', None),
        ...
        ('op_offload', 'Op Offload', 'bool', None),
        ('cpu_moe', 'CPU MoE', 'bool', None),
    ],
    'memory': [
        ('cache_type_k', 'KV Cache Type K', 'select', [('f16', 'f16'), ('f32', 'f32'), ('bf16', 'bf16'), ('q8_0', 'q8_0'), ('q4_0', 'q4_0'), ('q5_0', 'q5_0')]),
        ('cache_type_v', 'KV Cache Type V', 'select', [('f16', 'f16'), ('f32', 'f32'), ('bf16', 'bf16'), ('q8_0', 'q8_0'), ('q4_0', 'q4_0'), ('q5_0', 'q5_0')]),
        ('mmap', 'Memory Map', 'bool', None),
        ('mlock', 'Lock Memory', 'bool', None),
        ('direct_io', 'Direct I/O', 'bool', None),
    ],
    'sampling': [
        ...
        ('mirostat', 'Mirostat', 'select', [('0', 'disabled'), ('1', 'Mirostat'), ('2', 'Mirostat 2.0')]),
    ...
    'server': [
        ...
        ('webui', 'Web UI', 'bool', None),
        ('metrics', 'Prometheus Metrics', 'bool', None),
        ('slots', 'Slots Endpoint', 'bool', None),
    ...
    'speculative': [
        ...
        ('spec_type', 'Spec Type', 'select', [('none', 'none'), ('ngram-cache', 'ngram-cache'), ('ngram-simple', 'ngram-simple'), ('ngram-map-k', 'ngram-map-k')]),
    ...
    'chat_templates': [
        ...
        ('skip_chat_parsing', 'Skip Chat Parsing', 'bool', None),
        ('prefill_assistant', 'Prefill Assistant', 'bool', None),
    ],
    'checkpoints': [
        ('ctx_checkpoints', 'Context Checkpoints', 'int', 'default: 32'),
        ('cache_ram', 'Cache RAM (MiB)', 'int', '-1 = no limit'),
        ('kv_unified', 'Unified KV Buffer', 'bool', None),
        ('cache_idle_slots', 'Cache Idle Slots', 'bool', None),
    ],
    'logging': [
        ('log_verbosity', 'Verbosity', 'select', [('0', 'generic'), ('1', 'error'), ('2', 'warning'), ('3', 'info'), ('4', 'debug')]),
        ('log_file', 'Log File', 'text', None),
        ('log_colors', 'Log Colors', 'select', [('auto', 'auto'), ('on', 'on'), ('off', 'off')]),
        ('log_timestamps', 'Timestamps', 'bool', None),
        ('verbose', 'Verbose (All)', 'bool', None),
        ('offline', 'Offline Mode', 'bool', None),
    ],
    'advanced': [
        ('rope_scaling', 'RoPE Scaling', 'select', [('none', 'none'), ('linear', 'linear'), ('yarn', 'yarn')]),
...
COMPLEX_FIELDS = {
    ...
    'control_vectors': [('path', 'Path', 'text'), ('scale', 'Scale', 'float'),
                        ('layer_range_start', 'Layer Start', 'int'), ('layer_range_end', 'Layer End', 'int')],
    'override_kv': [('key_name', 'Key', 'text'), ('key_type', 'Type', 'select', [('int', 'int'), ('float', 'float'), ('bool', 'bool'), ('str', 'str')]), ('key_value', 'Value', 'text')],
    'override_tensors': [('tensor_pattern', 'Pattern', 'text'), ('buffer_type', 'Buffer Type', 'text')],
...
def render_field(category, col, value):
    ...
    if ftype == 'bool':
        ...
        return (f'<input type="password" name="{name}" value="{value or ""}" '
